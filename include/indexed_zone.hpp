#ifndef TIMEDREL_indexed_zone_HPP
#define TIMEDREL_indexed_zone_HPP 1
...
/*
The indexed_zone contains indices to track itself and the zones that were operated on to create it.
*/
template <class T>
class indexed_zone: public gen_zone{
    int myid;
    std::vector<int> chids;
    timedrel::zone<T> myzone = timedrel::zone<T>::make({0,0,0,0,0,0});
  public:
  	indexed_zone(timedrel::zone<T> &mz, int inmyid, int inchid1, int inchid2){
  	    this->myzone = mz;
  		this->myid   = inmyid;
        (this->chids).push_back(inchid1);
        (this->chids).push_back(inchid2);
  	}
    indexed_zone(const timedrel::zone<T> &mz, int inmyid, std::vector<int> chids){
        this->myzone = mz;
        this->myid = inmyid;
    ...
    void set_chids(std::vector<int> inchids){
  	...
  	virtual bool compare_less_bmin(const std::shared_ptr<gen_zone> &c){
  	    std::shared_ptr<indexed_zone<T>> iz = std::dynamic_pointer_cast<indexed_zone<T>>(c);
    ...
    virtual std::shared_ptr<gen_zone> clone(){
        return std::make_shared<indexed_zone<T>>(myzone, myid, chids);
    }
    virtual std::shared_ptr<gen_zone> intersection(const std::shared_ptr<gen_zone> &gz){
    	std::shared_ptr<indexed_zone<T>> iz = std::dynamic_pointer_cast<indexed_zone<T>>(gz);
    	timedrel::zone<T> izres = timedrel::zone<T>::intersection(this->get_myzone(), iz->get_myzone());
    	// Put the invalid -1 for myid
    	return std::make_shared<indexed_zone<T>>(izres, -1, this->get_myid(), iz->get_myid());
    }
    virtual std::shared_ptr<gen_zone> concatenation(const std::shared_ptr<gen_zone> &gz){
        // When concatenation we also concatenate the list of child ids
        std::shared_ptr<indexed_zone<T>> iz = std::dynamic_pointer_cast<indexed_zone<T>>(gz);
        ...
        return std::make_shared<indexed_zone<T>>(izres, -1, reschids1);
    }
...
#endif // TIMEDREL_indexed_zone_HPP
